...
let itemsTradesmanPotions = ITEM_RARITIES.map((rarity,i) => new Potion(i));
let itemsTradesmanBombs = ITEM_RARITIES.map((rarity,i) => new Bomb(i));
...
// UPDATE+D the function to return a random position on the board that is not occupied by an entity (Grass is fine) or the player's initial position (center)
...
// UPDATE+D the function passed to setInterval to attack the player and trigger player death if hp is 0 or lower
...
// UPDATE+D this function to getExp from monster, loot the monster, and clear the entity (monster) at the player position
...
// UPDATE+D the entity variable to be the entity at the player position
...
// UPDATE+D this function based on the comments
...
function createActionView(entity) {
  ...
  name.innerText = (entity instanceof Creature)? entity.name:entity.constructor.name;
...
// UPDATE+D this function to create the appropriate menu based on the entity type. Use the createMenu functions (e.g. createPickupMenu)
function createActionMenu(entity) {
  const actionMenu = document.createElement('div');
  actionMenu.id = 'action-menu';//alert(entity.constructor.name);
...
// UPDATE+D the pickupBtn event listener function to pickup the entity
...
// UPDATE+D this function to add a call to createItemActions(root, monster) if the player has items
...
function createMonsterMenu(root, monster) {
  ...
  attackBtn.addEventListener('click', () => {
    ...
    if (monster.hp<=0) {
      ...
      setTimeout(() => (attackBtn.disabled = false), player.attackSpeed);//1000);
...
function createItemActions(root, monster) {
  ...
  player.items.forEach((item) => { if(item instanceof Key) return;
    const itemBtn = document.createElement('button');
    itemBtn.innerText = `${item.constructor.name} - ${item.value}`;// Add code here to set the itemBtn text to the item name
    itemBtn.addEventListener('click', () => {
      (item.type === 'potion')? player.useItem(item, player): player.useItem(item, monster);
      if(monster.hp <=0) defeatMonster(monster);
      updateActionCam();      
    });
...
function createTradeMenu(root, tradesman) {
  ...
  tradesman.items.forEach((item) => {
    ...
    itemBtn.innerText = `${ITEM_RARITIES[item.rarity]} ${item.type} - ${item.value}G`;
...
// UPDATE+D the function based on the comments
...
function createDungeonMenu(root, dungeon) {
  ...
  if (!dungeon.isOpen) {
    ...
    if(!player.items.some((item)=> item.type==='key')) openBtn.disabled = true;
    openBtn.addEventListener('click', () => {
      remove(player.items, new Key());
      dungeon.isOpen = true;
      dungeon.element.src = 'imgs/dungeon/open.png';
      onceOpened(root,actions,dungeon);      
      updateActionCam();
    });
...
function onceOpened(root,actions,dungeon){
  if (dungeon.hasPrincess) {
    ...
    lootBtn.disabled = dungeon.gold<=0 && dungeon.items.length<=0;
